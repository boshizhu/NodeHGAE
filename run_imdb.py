...
from load_data import load_data_IMDB
...
def main(args):
    ...
    g, train_idx, val_idx, test_idx,ADJ_TOPO,ADJ = load_data_IMDB()
    ADJ = [graph.to(args['device']) for graph in ADJ]
    z_pre = np.load(
        'D:\代码下载\异质图代码\HGNN-AC-main\HGNN-AC-main\data\preprocessed\IMDB_processed\metapath2vec_emb.npy')
    z_pre = torch.FloatTensor(z_pre).cuda()
    z_pre = z_pre[0:4278, :]
    topo_dim = ADJ_TOPO.shape[1]
    ADJ_TOPO = ADJ_TOPO.to(args['device'])
    featsdim=g.ndata["feature"]['movie'].shape[1]
    in_dims = [g.ndata["feature"][tp].shape[1] for tp in g.ntypes]
    labels = g.ndata['label']['movie']
    labels = labels.to(args['device'])
    numclass, _ = torch.max(g.ndata['label']['movie'], dim=0)
    print(numclass.item() + 1)
    g = g.to(args['device'])
    print(g.canonical_etypes)
    x_ture = g.ndata["feature"]['movie']
    ...
    #==========这里暂时设置超参数
    in_feats = {'movie': 3066, 'director': 3066, 'actor': 3066}
    hidden_feats = 64
    out_feats = numclass.item() + 1
    num_layers = 2
    aggregator_type_dict = {'movie': 'mean', 'director': 'gcn','actor': 'gcn'}
    #==========
    ...
    for cur_repeat in range(args['repeat']):
        ...
        for epoch in range(args['num_epochs']):
            mask_v1, _ = get_feat_mask(g.ndata['feature'][target], 0.4)  # 设置锚视角下的特征遮蔽
    ...
    nmi_avg /= args['repeat']
    ari_avg /= args['repeat']
    with open('results_r_imdb.txt', 'a') as file:
...
if __name__ == '__main__':
    ...
    #
    #         # parser.add_argument('--w_Local', default=1, help='属性重构损失')  # 1  #julei 3 0.2 0.2
    #         # parser.add_argument('--w_ToPo', default=0.7, help='局部拓扑重构损失')  # 0.7
    #         # parser.add_argument('--w_High', default=0.7 , help='元路径重构损失')  # 0.7
    #         parser.add_argument('--w_Local', default=0 + 0.1 * i, help='属性重构损失')  # 0.8
    #         parser.add_argument('--w_High', default=0 + 0.1 * j, help='边缘重构损失')  # 0.1
    #         parser.add_argument('--w_ToPo', default=0 + 0.1 * j, help='局部拓扑重构损失')  # 0.1
    #
