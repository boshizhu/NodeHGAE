...
from load_data import load_data_DBLP
...
def main(args):
    ...
    g, train_idx, val_idx, test_idx,ADJ_TOPO,ADJ = load_data_DBLP()
    ADJ = [graph.to(args['device']) for graph in ADJ]
    z_pre = np.load(r'D:\daima\NodeHGAE\DBLP\metapath2vec_emb.npy')
    z_pre = torch.FloatTensor(z_pre).cuda()
    z_pre = z_pre[0:4057, :]
    topo_dim = ADJ_TOPO.shape[1]
    ADJ_TOPO = ADJ_TOPO.to(args['device'])
    featsdim=g.ndata["feature"]['author'].shape[1]
    in_dims = [g.ndata["feature"][tp].shape[1] for tp in g.ntypes]
    labels = g.ndata['label']['author']
    labels = labels.to(args['device'])
    numclass, _ = torch.max(g.ndata['label']['author'], dim=0)
    print(numclass.item() + 1)
    g = g.to(args['device'])
    print(g.canonical_etypes)
    x_ture = g.ndata["feature"]['author']
    ...
    in_feats = {'author': 334, 'paper': 4231, 'term': 50,'conferance': 20}
    hidden_feats = 64
    out_feats = numclass.item() + 1
    num_layers = 2
    aggregator_type_dict = {'paper': 'mean', 'author': 'gcn','conferance': 'gcn','term': 'gcn'}
    #==========
    target = 'author'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--w_Local', default=1, help='属性重构损失')#0.3
    parser.add_argument('--w_High', default=0.9, help='元路径重构损失')#0.8
    parser.add_argument('--w_ToPo', default=0.9, help='局部拓扑重构损失')#0.8
