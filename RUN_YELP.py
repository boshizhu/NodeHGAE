...
from load_data import load_data_YELP
...
def main(args):
    ...
    g, train_idx, val_idx, test_idx,ADJ_TOPO,ADJ = load_data_YELP()
    ADJ = [graph.to(args['device']) for graph in ADJ]
    topo_dim = ADJ_TOPO.shape[1]
    z_pre1=sp.load_npz("D:\工作\任务\yelp\matepath2vec_blb.npz").A
    z_pre2 = sp.load_npz("D:\工作\任务\yelp\matepath2vec_bub.npz").A
    z_pre3 = sp.load_npz("D:\工作\任务\yelp\matepath2vec_bsb.npz").A
    z_pre1 = torch.FloatTensor(z_pre1).cuda()
    z_pre2 = torch.FloatTensor(z_pre2).cuda()
    z_pre3 = torch.FloatTensor(z_pre3).cuda()
    z_pre=torch.cat((z_pre1,z_pre2,z_pre3),1)
    z_pre=z_pre.cuda()
    z_pre=z_pre[0:2614,:]
    ADJ_TOPO = ADJ_TOPO.to(args['device'])
    featsdim=g.ndata["feature"]['business'].shape[1]
    in_dims = [g.ndata["feature"][tp].shape[1] for tp in g.ntypes]
    labels = g.ndata['label']['business']
    labels = labels.to(args['device'])
    numclass, _ = torch.max(g.ndata['label']['business'], dim=0)
    print(numclass.item() + 1)
    g = g.to(args['device'])
    print(g.canonical_etypes)
    x_ture = g.ndata["feature"]['business']
    ...
    in_feats = {'business': 82, 'users': 82, 'leval': 82,'sevice': 82}
    hidden_feats = 64
    out_feats = numclass.item() + 1
    num_layers = 2
    aggregator_type_dict = {'business': 'mean', 'users': 'gcn','leval': 'gcn','sevice': 'gcn'}
    #==========
    ...
    for cur_repeat in range(args['repeat']):
        ...
        model = HeteroGraphSAGE(g, in_feats, hidden_feats, out_feats, num_layers, aggregator_type_dict, featsdim,
                                topo_dim, args['dataset'],args['alpha'],args['Init'],args['rank'],args['K']).to(args['device'])
        loss_fcn = torch.nn.CrossEntropyLoss()
        early_stopping = EarlyStopping(patience=args['patience'], verbose=True,
                                       save_path='checkpoint_{}.pt'.format(args['dataset']))  # 提早停止，设置的耐心值为5
        ...
        x1 = (h.detach().cpu().numpy())
        np.save('x_acm_cchgae', x1)
        svm_macro, svm_micro, nmi, ari = evaluate_results_nc(h[test_idx].detach().cpu().numpy(),
    ...
    nmi_avg /= args['repeat']
    ari_avg /= args['repeat']
    with open('results_r_yelp.txt', 'a') as file:
        file.write(str(args['dataset']) + '\n')
        file.write(str('w1')+str(':')+str(args['w_Local'])+ '\n')
        file.write(str('w2')+str(':')+str(args['w_High']) + '\n')
        file.write(str('w2')+str(':')+str(args['w_ToPo']) + '\n')
        file.write(str('Macro-F1:')+str(svm_macro_avg) + '\n')
...
if __name__ == '__main__':
    # for i in range(0, 11):
    #     for j in range(0, 11):
    #         parser = argparse.ArgumentParser(description='这是我们基于关系耦合所构建的CCHGAE模型')
    ...
    #         parser.add_argument('--num_epochs', default=10000, help='最大迭代次数')
    #         parser.add_argument('--weight_decay', default=0.0001, help='权重衰减')
    #         parser.add_argument('--patience', type=int, default=30, help='耐心值')
    #         parser.add_argument('--seed', type=int, default=123, help='随机种子')
    #
    #         # parser.add_argument('--w_Local', default=0.1, help='属性重构损失')  # 0.1
    #         # parser.add_argument('--w_High', default=0.4, help='子图重构损失')  # 0.4
    #         # parser.add_argument('--w_ToPo', default=0.4, help='位置重构损失')  # 0.4
    #         parser.add_argument('--w_Local', default=0 + 0.1 * i, help='属性重构损失')
    #         parser.add_argument('--w_High', default=0 + 0.1 * j, help='边缘重构损失')
    #         parser.add_argument('--w_ToPo', default=0 + 0.1 * j, help='局部拓扑重构损失')
    #         parser.add_argument('--w_cov', default=1, help='协方差')
    ...
    #         parser.add_argument('--Init', type=str, default='Mine_PPR', help='重复训练和测试次数')
    #         parser.add_argument('--rank', type=int, default=3, help='秩')
    #         parser.add_argument('--K', type=int, default=3, help='卷积层数')
    ...
    parser.add_argument('--seed', type=int, default=123,help='随机种子')
    parser.add_argument('--w_Local', default=0.1 , help='属性重构损失')#0.1
    parser.add_argument('--w_High', default=0.3, help='子图重构损失')#0.4
    parser.add_argument('--w_ToPo', default=0.3, help='位置重构损失')#0.4
    parser.add_argument('--w_cov', default=1, help='协方差')
    parser.add_argument('--device', type=str, default='cuda:0', help='使用cuda:0或者cpu')
    parser.add_argument('--repeat', type=int, default=1, help='重复训练和测试次数')
    parser.add_argument('--alpha', type=int, default=0.25, help='')
